...
MIN_BULGE_RATIO = 0.005
MAX_BULGE_RATIO = 0.25
BULGE_REGION_SUBDIV_CUTS = 2
DEFAULT_BULGE_MIN_HEIGHT_RATIO = 0.10
DEFAULT_BULGE_MAX_HEIGHT_RATIO = 0.14
DEFAULT_BULGE_MASK_SCALE = 1.0
DEFAULT_BULGE_SHORT_AXIS_MIN_RATIO = 0.055
DEFAULT_BULGE_SHORT_AXIS_MAX_RATIO = 0.095
DEFAULT_BULGE_LONG_AXIS_MIN_RATIO = DEFAULT_BULGE_SHORT_AXIS_MAX_RATIO
DEFAULT_BULGE_LONG_AXIS_MAX_RATIO = 0.3
DEFAULT_BULGE_LONG_AXIS_PLATEAU_RATIO = 0.55
DEFAULT_BULGE_EDGE_TAPER_START = 0.88
DEFAULT_WHOLE_MESH_SUBDIV_CUTS = 2
GENERATED_UV_NAME = "BulgeTextureUV"
...
def prepare_texture_mapping(obj):
    ...
    if uv_layer is None:
        ...
        obj["_bulge_generated_uvs"] = False
...
def quadratic_edge_taper(r, start=DEFAULT_BULGE_EDGE_TAPER_START):
...
def choose_bulge_seed(target_obj, camera=None,
                      bulge_min_height_ratio=DEFAULT_BULGE_MIN_HEIGHT_RATIO,
                      bulge_max_height_ratio=DEFAULT_BULGE_MAX_HEIGHT_RATIO,
                      bulge_mask_scale=DEFAULT_BULGE_MASK_SCALE,
                      short_axis_min_ratio=DEFAULT_BULGE_SHORT_AXIS_MIN_RATIO,
                      short_axis_max_ratio=DEFAULT_BULGE_SHORT_AXIS_MAX_RATIO,
                      long_axis_min_ratio=DEFAULT_BULGE_LONG_AXIS_MIN_RATIO,
                      long_axis_max_ratio=DEFAULT_BULGE_LONG_AXIS_MAX_RATIO,
                      long_axis_plateau_ratio=DEFAULT_BULGE_LONG_AXIS_PLATEAU_RATIO):
    ...
    mask_scale = max(bulge_mask_scale, 0.05)
    min_height_ratio, max_height_ratio = normalized_ratio_range(
        bulge_min_height_ratio,
        bulge_max_height_ratio,
        DEFAULT_BULGE_MIN_HEIGHT_RATIO,
        DEFAULT_BULGE_MAX_HEIGHT_RATIO,
    )

    short_min, short_max = normalized_ratio_range(
        short_axis_min_ratio,
        short_axis_max_ratio,
        DEFAULT_BULGE_SHORT_AXIS_MIN_RATIO,
        DEFAULT_BULGE_SHORT_AXIS_MAX_RATIO,
    )
    long_min, long_max = normalized_ratio_range(
        long_axis_min_ratio,
        long_axis_max_ratio,
        DEFAULT_BULGE_LONG_AXIS_MIN_RATIO,
        DEFAULT_BULGE_LONG_AXIS_MAX_RATIO,
    )
...
def bulge_weight(world_co, params):
...
def bulge_planar_weight(world_co, params):
    delta = world_co - params["center"]
    u = delta.dot(params["tangent_u"]) / params["axis_a"]
    v = delta.dot(params["tangent_v"]) / params["axis_b"]
    r2 = u * u + v * v
...
def candidate_bulge_faces(obj, params):
    mesh = obj.data
    face_indices = []
    for poly in mesh.polygons:
        coords = [obj.matrix_world @ mesh.vertices[idx].co for idx in poly.vertices]
        center = sum(coords, Vector((0.0, 0.0, 0.0))) / len(coords)
        if bulge_weight(center, params) > 0.0:
            face_indices.append(poly.index)
            continue
        if any(bulge_weight(co, params) > 0.0 for co in coords):
            face_indices.append(poly.index)
    return face_indices


def subdivide_bulge_region(obj, face_indices, cuts=BULGE_REGION_SUBDIV_CUTS):
    ...
    bm.free()
    if obj.get("_bulge_generated_uvs"):
...
def create_constant_displace_texture():
    image = bpy.data.images.new("Bulge_displace_white_image", width=1, height=1, alpha=True)
    image.pixels[0:4] = (1.0, 1.0, 1.0, 1.0)

    texture = bpy.data.textures.new("Bulge_displace_white_texture", type="IMAGE")
    texture.image = image
    return texture


def create_bulge_vertex_group(target_obj, params):
    mesh = target_obj.data
    vertex_group = target_obj.vertex_groups.new(name="Bulge_Displace_Weight")
    selected_vertices = set()
    vertex_weights = {}

    for vert in mesh.vertices:
        world_co = target_obj.matrix_world @ vert.co
        weight = bulge_weight(world_co, params)
        if weight <= 0.0:
            continue
...
def collect_bulge_faces(target_obj, params, vertex_weights):
    mesh = target_obj.data
    face_weights = {}
    ...
    for poly in mesh.polygons:
        ...
        center = sum(coords, Vector((0.0, 0.0, 0.0))) / len(coords)
        weight = bulge_weight(center, params)
        if weight <= 0.0:
...
def apply_bulge_displace_modifier(target_obj, vertex_group, params):
    texture = create_constant_displace_texture()
    modifier = target_obj.modifiers.new("Bulge_Displace", "DISPLACE")
...
def deform_target_to_bulge(target_obj, params):
    vertex_group, selected_vertices, vertex_weights = create_bulge_vertex_group(target_obj, params)
    if not selected_vertices:
        return selected_vertices, [], vertex_weights, {}

    face_weights = collect_bulge_faces(target_obj, params, vertex_weights)
    apply_bulge_displace_modifier(target_obj, vertex_group, params)

    selected_faces = sorted(face_weights.keys())
    return selected_vertices, selected_faces, vertex_weights, face_weights


def create_bulge_mask_object(target_obj, selected_faces, offset, push_normal):
    mesh = target_obj.data
    ...
    mask_mesh = bpy.data.meshes.new("Bulge_mask_mesh")
    mask_mesh.from_pydata(verts, [], faces)
    mask_mesh.update()
    mask_obj = bpy.data.objects.new("Bulge_mask_region", mask_mesh)
...
def build_bulge_scene(parts, largest_k, camera=None,
                      bulge_min_height_ratio=DEFAULT_BULGE_MIN_HEIGHT_RATIO,
                      bulge_max_height_ratio=DEFAULT_BULGE_MAX_HEIGHT_RATIO,
                      bulge_mask_scale=DEFAULT_BULGE_MASK_SCALE,
                      short_axis_min_ratio=DEFAULT_BULGE_SHORT_AXIS_MIN_RATIO,
                      ...
                      bulge_region_subdiv_cuts=BULGE_REGION_SUBDIV_CUTS):
    if not parts:
        return None
    ...
    params = choose_bulge_seed(
        target_obj,
        camera,
        bulge_min_height_ratio=bulge_min_height_ratio,
        bulge_max_height_ratio=bulge_max_height_ratio,
        bulge_mask_scale=bulge_mask_scale,
    ...
    face_indices = candidate_bulge_faces(target_obj, params)
    if not face_indices:
        return None
    subdivide_bulge_region(target_obj, face_indices, bulge_region_subdiv_cuts)

    selected_vertices, selected_faces, _, _ = deform_target_to_bulge(target_obj, params)
    if not selected_vertices or not selected_faces:
        return None

    mask_obj = create_bulge_mask_object(
        target_obj,
        selected_faces,
    ...
    bulged_parts = [item["object"] for item in parts]
    return {
        "bulged_parts": bulged_parts,
        "mask_obj": mask_obj,
        "target_source": target_info["source"],
        "all_for_camera": bulged_parts,
        "bulge_axis_a": params["axis_a"],
        "bulge_axis_b": params["axis_b"],
        "bulge_long_axis": params["long_axis"],
        "bulge_short_axis": params["short_axis"],
        "bulge_long_axis_ratio": params["long_axis_ratio"],
        "bulge_short_axis_ratio": params["short_axis_ratio"],
        "bulge_long_axis_plateau_ratio": params["long_axis_plateau_ratio"],
        "bulge_height": params["height"],
        "bulge_height_ratio": params["height_ratio"],
        "bulge_min_height_ratio": params["min_height_ratio"],
        "bulge_max_height_ratio": params["max_height_ratio"],
        "bulge_mask_scale": params["mask_scale"],
        "bulge_deformation_method": "displace_modifier_vertex_group",
        "whole_mesh_subdiv_cuts": whole_mesh_subdiv_cuts,
        "bulge_region_subdiv_cuts": bulge_region_subdiv_cuts,
        "bulge_center": list(params["center"]),
        "bulge_normal": list(params["normal"]),
    }


def render_bulge_mask(path, bulged_parts, mask_obj, mode):
    white = create_emission_material("Bulge_mask_white", (1, 1, 1, 1))
    black = create_emission_material("Bulge_mask_black", (0, 0, 0, 1))

    if mode == "bulge":
        for obj in bulged_parts:
...
def save_bulge_json(image_path, camera, scene_data, radius, elevation, azimuth, ratio):
    ...
    data = {
        ...
        "RT_": rt_,
        "anomaly_type": "Bulge",
        "bulge_part": os.path.basename(scene_data["target_source"]),
        "bulge_mask_ratio": ratio,
        "bulge_axis_a": scene_data["bulge_axis_a"],
        "bulge_axis_b": scene_data["bulge_axis_b"],
        "bulge_long_axis": scene_data["bulge_long_axis"],
        "bulge_short_axis": scene_data["bulge_short_axis"],
        "bulge_long_axis_ratio": scene_data["bulge_long_axis_ratio"],
        "bulge_short_axis_ratio": scene_data["bulge_short_axis_ratio"],
        "bulge_long_axis_plateau_ratio": scene_data["bulge_long_axis_plateau_ratio"],
        "bulge_height": scene_data["bulge_height"],
        "bulge_height_ratio": scene_data["bulge_height_ratio"],
        "bulge_min_height_ratio": scene_data["bulge_min_height_ratio"],
        "bulge_max_height_ratio": scene_data["bulge_max_height_ratio"],
        "bulge_mask_scale": scene_data["bulge_mask_scale"],
        "bulge_deformation_method": scene_data["bulge_deformation_method"],
        "whole_mesh_subdiv_cuts": scene_data["whole_mesh_subdiv_cuts"],
        "bulge_region_subdiv_cuts": scene_data["bulge_region_subdiv_cuts"],
        "bulge_center_world": scene_data["bulge_center"],
        "bulge_normal_world": scene_data["bulge_normal"],
        "radius": radius,
...
def render_bulge_pair(shape_id, output_dir, scene_data, camera, radius, elevation, azimuth,
                      min_bulge_ratio, max_bulge_ratio, final_samples):
    bulged_parts = scene_data["bulged_parts"]
    ...
    stem = f"render_{shape_id}_{radius}_{az_deg}_{el_deg}_Bulge_anomaly"
    ...
    material_map = {obj.name: list(obj.data.materials) for obj in bulged_parts + [mask_obj]}

    render_bulge_mask(mask_path, bulged_parts, mask_obj, "bulge")
    render_bulge_mask(complete_mask_path, bulged_parts, mask_obj, "complete")

    bulge_pixels = count_mask_pixels(mask_path)
    complete_pixels = count_mask_pixels(complete_mask_path)
    remove_file_if_exists(complete_mask_path)

    ratio = bulge_pixels / complete_pixels if complete_pixels > 0 else 0.0
    if ratio < min_bulge_ratio or ratio > max_bulge_ratio:
        remove_file_if_exists(mask_path)
        restore_materials(bulged_parts + [mask_obj], material_map)
        return False, ratio
    ...
    save_bulge_json(image_path, camera, scene_data, radius, elevation, azimuth, ratio)
    return True, ratio
...
def parse_args():
    ...
    parser = ArgumentParser(description="Render local bulged-part anomaly images and masks.")
    ...
    parser.add_argument("--min_bulge_ratio", type=float, default=MIN_BULGE_RATIO)
    parser.add_argument("--max_bulge_ratio", type=float, default=MAX_BULGE_RATIO)
    parser.add_argument("--bulge_min_height_ratio", type=float, default=DEFAULT_BULGE_MIN_HEIGHT_RATIO,
                        help="Minimum bulge peak height as a ratio of the target part max dimension.")
    parser.add_argument("--bulge_max_height_ratio", type=float, default=DEFAULT_BULGE_MAX_HEIGHT_RATIO,
                        help="Maximum bulge height as a ratio of the target part max dimension.")
    parser.add_argument("--bulge_mask_scale", type=float, default=DEFAULT_BULGE_MASK_SCALE,
                        help="Scale factor for the elliptical bulge influence region and mask size.")
    parser.add_argument("--bulge_short_axis_min_ratio", type=float,
                        default=DEFAULT_BULGE_SHORT_AXIS_MIN_RATIO,
                        help="Minimum short-axis length as a ratio of the target part max dimension.")
    parser.add_argument("--bulge_short_axis_max_ratio", type=float,
                        default=DEFAULT_BULGE_SHORT_AXIS_MAX_RATIO,
                        help="Maximum short-axis length as a ratio of the target part max dimension.")
    parser.add_argument("--bulge_long_axis_min_ratio", type=float,
                        default=DEFAULT_BULGE_LONG_AXIS_MIN_RATIO,
                        help="Minimum long-axis length as a ratio of the target part max dimension.")
    parser.add_argument("--bulge_long_axis_max_ratio", type=float,
                        default=DEFAULT_BULGE_LONG_AXIS_MAX_RATIO,
                        help="Maximum long-axis length as a ratio of the target part max dimension.")
    parser.add_argument("--bulge_long_axis_plateau_ratio", type=float,
                        default=DEFAULT_BULGE_LONG_AXIS_PLATEAU_RATIO,
                        help="Flat high-bulge fraction along the normalized long-axis half-length.")
    ...
    parser.add_argument("--bulge_region_subdiv_cuts", type=int, default=BULGE_REGION_SUBDIV_CUTS,
                        help="Optional extra subdivision cuts applied only to the bulge candidate region.")
...
def main():
    ...
    image_root = os.path.join(root, "Bulge", args.category)
    ...
    for shape in shapes:
        ...
        for view_idx in range(args.num_views):
            ...
            for attempt in range(args.max_attempts):
                ...
                scene_data = build_bulge_scene(
                    parts,
                    args.largest_k,
                    camera,
                    bulge_min_height_ratio=args.bulge_min_height_ratio,
                    bulge_max_height_ratio=args.bulge_max_height_ratio,
                    bulge_mask_scale=args.bulge_mask_scale,
                    short_axis_min_ratio=args.bulge_short_axis_min_ratio,
                    short_axis_max_ratio=args.bulge_short_axis_max_ratio,
                    long_axis_min_ratio=args.bulge_long_axis_min_ratio,
                    long_axis_max_ratio=args.bulge_long_axis_max_ratio,
                    long_axis_plateau_ratio=args.bulge_long_axis_plateau_ratio,
                    whole_mesh_subdiv_cuts=args.whole_mesh_subdiv_cuts,
                    bulge_region_subdiv_cuts=args.bulge_region_subdiv_cuts,
                )
                if scene_data is None:
                    last_ratio = 0.0
                    continue

                ok, last_ratio = render_bulge_pair(
                    shape_id,
                    image_shape_path,
                    scene_data,
                    camera,
                    ...
                    elevation,
                    azimuth,
                    args.min_bulge_ratio,
                    args.max_bulge_ratio,
                    args.samples,
                )
                if ok:
                    print(
                        ...
                        f"short_axis={scene_data['bulge_short_axis']:.4f} "
                        f"long_axis={scene_data['bulge_long_axis']:.4f} "
                        f"long_plateau={scene_data['bulge_long_axis_plateau_ratio']:.2f} "
                        f"height={scene_data['bulge_height']:.4f}"
                    )
                    break

            if not ok:
                print(f"[SKIP] no valid bulge: {shape} view={view_idx} last_ratio={last_ratio:.3f}")
